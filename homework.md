...
if (($? != 0))  
then  
date >> curl.log	# здесь вместо ">>" лучше поставить ">" так место на диске не будет заполняться, а по единственной записи времени можно будет определить упал сервис или нет.
